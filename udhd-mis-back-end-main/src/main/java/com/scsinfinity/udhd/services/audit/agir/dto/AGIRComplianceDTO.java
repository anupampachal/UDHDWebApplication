...
@SuperBuilder
@NoArgsConstructor
public class AGIRComplianceDTO extends AuditComplianceDTO {


	@JsonIgnore
	public AuditComplianceEntity getEntity(AuditCriteriaEntity criteria) {
		return AuditComplianceEntity.builder().id(getId()).comment(getComment()).status(getStatus()).criteria(criteria).file(getFile()).build();
	}
}
