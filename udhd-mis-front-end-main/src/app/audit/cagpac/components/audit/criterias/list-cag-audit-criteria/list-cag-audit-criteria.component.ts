...
import { CAGAuditComplianceService } from '../compliances/compliance.service';
import { CAGAuditCriteriaService } from '../criteria.service';
import { CAGAuditCommentService } from '../comments/comments.service';
import { AuditCommentDTO } from 'src/app/audit/cagpac/models/cag-comment.model';

@Component({
  selector: 'app-list-cag-audit-criteria',
  templateUrl: './list-cag-audit-criteria.component.html',
  styleUrls: ['./list-cag-audit-criteria.component.css']
})
export class ListCagAuditCriteriaComponent implements OnInit {
  @Input() parent!: string;
  @Input() id!: number;
  @Input() status!: string;
  ...
  clearDataEvent(data: PaginationModel) {
    this.filterdata = '';
    this.paginationTableData$ = this.cagAuditCriteriaService
      .getCAGAuditCriteriaByPage(data)
  ...
  changeDataForPagination($event: PaginatorModel) {
    ...
    this.paginationTableData$ = this.cagAuditCriteriaService
      .getCAGAuditCriteriaByPage(page)
      .pipe(map(this.setTableData));
  }
  ...
  ngOnInit() {
   /* this.cagAuditComplianceService
    ...
    /*  this.cagAuditCommentService.getCAGAuditCommentByPage(
        this.getPageModel(
          'ALL',
          0,
          10,
