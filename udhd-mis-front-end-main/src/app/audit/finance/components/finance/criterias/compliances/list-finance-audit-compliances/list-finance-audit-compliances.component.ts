...
@Component({
  selector: 'app-list-finance-audit-compliances',
  templateUrl: './list-finance-audit-compliances.component.html',
  styleUrls: ['./list-finance-audit-compliances.component.css']
})
export class ListFinanceAuditCompliancesComponent implements OnInit {
  ...
  constructor(private financeAuditComplianceService: FinanceAuditComplianceService,
  ...
  ngOnInit(): void {
    this.paginationTableData$ = this.openedAccordion.pipe(
      ...
      switchMap(() =>
        this.financeAuditComplianceService
          .getFinanceAuditComplianceByPage(
            this.getPageModel(
              'ALL',
              0,
              10,
  ...
  paginationTableData$: Observable<MatTableWithPagination<AuditComplianceDTO>> =
    of();
  ...
  clearDataEvent(data: PaginationModel) {
    this.filterdata = '';
    this.paginationTableData$ = this.financeAuditComplianceService
      .getFinanceAuditComplianceByPage(data)
  ...
  showView($event: AuditComplianceDTO) {
    ...
    this.changeComponent.emit($event.id);
  }
  ...
  changeDataForPagination($event: PaginatorModel) {
    ...
    this.paginationTableData$ = this.financeAuditComplianceService
      .getFinanceAuditComplianceByPage(page)
      .pipe(map(this.setTableData));
  }
  ...
  setTableData(data: GenericResponseObject<AuditComplianceDTO>) {
