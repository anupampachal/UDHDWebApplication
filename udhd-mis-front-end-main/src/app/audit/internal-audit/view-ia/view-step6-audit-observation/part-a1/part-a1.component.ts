...
@Component({
  selector: 'app-part-a1',
  templateUrl: './part-a1.component.html',
  styleUrls: ['./part-a1.component.css']
})
export class PartA1Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.pipe(
      ...
      switchMap((param) => this.service.getAuditObsPartAlineItemByType(param['id'], "EXCESS_PAYMENT_AGAINST_BILL")),
  ...
  deleteObjective(i: number) {
    ...
    this.service.deleteAuditObsById(idToDelete, this.service1.step1Report.id, "EXCESS_PAYMENT_AGAINST_BILL").subscribe(() => {
...
partAId: number[] = []
  ...
  //     partAType: "EXCESS_PAYMENT_AGAINST_BILL",
  ...
  //       return this.service.uploadFileForLineItem("EXCESS_PAYMENT_AGAINST_BILL", res.id, this.service1.step1Report.id, this.selectedFile);
  //       }
  //       return of(res)
  //     }),
  //     catchError(err => {
  ...
  downloadObjectiveFile(i: number) {
    this.route.params.pipe(
      filter((params) => params && params['id']),
      switchMap((param) => this.service.getFileById(param['id'], "EXCESS_PAYMENT_AGAINST_BILL")),
      tap((res) => {
        if(res.size == 0) {
