...
import { AuditCommentDTO } from '../../../../../models/agircomment.model';
...
@Component({
  selector: 'app-list-agir-audit-comments',
  templateUrl: './list-agir-audit-comments.component.html',
  styleUrls: ['./list-agir-audit-comments.component.css']
})
export class ListAgirAuditCommentsComponent implements OnInit {
  ...
  clearDataEvent(data: PaginationModel) {
    this.filterdata = '';
    this.paginationTableData$ = this.agirAuditCommentsService
      .getAGIRAuditCommentByPage(data)
  ...
  changeDataForPagination($event: PaginatorModel) {
    ...
    this.paginationTableData$ = this.agirAuditCommentsService
      .getAGIRAuditCommentByPage(page)
      .pipe(map(this.setTableData));
  }
