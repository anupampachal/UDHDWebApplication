...
@Component({
  selector: 'app-create-udhd-user',
  templateUrl: './create-udhd-user.component.html',
  styleUrls: ['./create-udhd-user.component.css']
})
export class CreateUdhdUserComponent implements OnInit {
  authList:AuthEntity[]=[];
  createUDHDUserFG!: FormGroup;
  ...
  constructor(
    ...
    private router: Router){
      this.createUDHDUserFG = this.fb.group({
      ...
      this.createUDHDUserFG.addValidators(
        this.matchValidator(this.createUDHDUserFG.controls['password'], 
        this.createUDHDUserFG.controls['confirmPassword'])
      );
    }
  ...
  onSubmit() {
    if (this.createUDHDUserFG.valid) {
      this.userService
        .createUpdateUDHDUser(this.createUDHDUserFG.getRawValue())
        .subscribe((res) =>
          this.router.navigate(['/settings/settings-home/user-mgt/usermgt/view/', res.id], {
            relativeTo: this.route,
          })
        );
    } else if (!!this.createUDHDUserFG.errors) {
      this.showFormError = this.createUDHDUserFG.errors;
    }
  }
  ...
  getErrorMessages(controls:string){
    ...
    let control= this.createUDHDUserFG.controls[controls]
